...
@section('title', 'Tambah Akun Staff Baru - SIRAB Kelompok-3')
...
@section('content')
  ...
  <div class="text-xs text-slate-500 mb-6 flex items-center gap-1.5">
    ...
    <span>/</span>
    <span class="text-slate-800 font-medium">Tambah Staff Baru</span>
  </div>
  ...
  <div class="max-w-3xl mx-auto">
    ...
    <div class="flex items-center justify-between bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-6">
      <div>
        ...
        <h1 class="text-2xl font-bold text-slate-900">Tambah Akun Staff Baru</h1>
        <p class="text-sm text-slate-500 mt-1">
          Daftarkan pengguna baru dengan hak akses pemohon RAB dan tentukan unit kerja divisinya.
        </p>
      </div>
      ...
      <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <span class="text-[10px] text-slate-400 mt-1 block">Minimal 6 karakter untuk keamanan akun.</span>
            @error('password')
              <p class="mt-1.5 text-xs text-rose-600 flex items-center gap-1">
                <span>⚠</span> {{ $message }}
              </p>
        ...
        <div class="p-4 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between text-xs">
          <div class="flex items-center gap-2">
            ...
            <span class="text-slate-600">Otomatis ditetapkan sebagai</span>
          ...
          <span class="text-[10px] text-slate-400">Tunduk pada RBAC Middleware</span>
        </div>
