...
@section('title', 'Tambah Akun Pengguna Baru - Administrator IT')
...
@section('content')
  ...
  <div class="max-w-2xl mx-auto bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm">
    <div class="mb-6 border-b border-slate-100 pb-4">
      ...
      <p class="text-xs text-slate-500 mt-1">Daftarkan akun dengan salah satu dari 4 role: Staff, Finance, Pimpinan, atau Admin IT.</p>
    </div>

    <form action="{{ route('admin-it.users.store') }}" method="POST" class="space-y-4">
      @csrf

      <div>
        <label class="block text-xs font-semibold text-slate-700 mb-1">Nama Lengkap <span class="text-rose-500">*</span></label>
        <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap') }}" required class="w-full px-3.5 py-2 border border-slate-300 rounded-xl text-sm focus:border-indigo-500">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="email" name="email" value="{{ old('email') }}" required class="w-full px-3.5 py-2 border border-slate-300 rounded-xl text-sm font-mono focus:border-indigo-500">
          @error('email') <p class="text-xs text-rose-600 mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
          <label class="block text-xs font-semibold text-slate-700 mb-1">Password <span class="text-rose-500">*</span></label>
          <input type="password" name="password" minlength="6" required placeholder="Min 6 karakter" class="w-full px-3.5 py-2 border border-slate-300 rounded-xl text-sm focus:border-indigo-500">
      ...
      <div>
        ...
        <input type="text" name="jabatan" value="{{ old('jabatan') }}" placeholder="Contoh: Staf IT / Reviewer Finance / Kepala Divisi" class="w-full px-3.5 py-2 border border-slate-300 rounded-xl text-sm focus:border-indigo-500">
      </div>
      ...
      <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
        ...
        <button type="submit" class="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-semibold">Simpan Akun</button>
      </div>
    </form>
  </div>
@endsection
